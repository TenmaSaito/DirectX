...
void InitTimer(void)
{
	...
	g_posTimer = D3DXVECTOR3(500, 50, 0);		// 位置を初期化
	g_nTimer = 0;								// 値を初期化
	...
	{
		// 頂点座標の設定(座標設定は必ず右回りで！！！)
		pVtx[0].pos.x = g_posTimer.x + (70.0f * nCntTimer) - 30.0f;
		pVtx[0].pos.y = g_posTimer.y - 40.0f;
		pVtx[0].pos.z = 0.0f;

		pVtx[1].pos.x = g_posTimer.x + (70.0f * nCntTimer) + 30.0f;
		pVtx[1].pos.y = g_posTimer.y - 40.0f;
		pVtx[1].pos.z = 0.0f;

		pVtx[2].pos.x = g_posTimer.x + (70.0f * nCntTimer) - 30.0f;
		pVtx[2].pos.y = g_posTimer.y + 40.0f;
		pVtx[2].pos.z = 0.0f;

		pVtx[3].pos.x = g_posTimer.x + (70.0f * nCntTimer) + 30.0f;
...
void SetTimer(int nTimer)
{
	...
	// タイマーと同期したゲージを作成
	g_nGaugeTimer = SetGauge(D3DXVECTOR3(g_posTimer.x - 80.0f, g_posTimer.y, g_posTimer.z), TIMERGAUGE_COLOR, GAUGETYPE_TIMER, GAUGE_MAX, 40.0f);
